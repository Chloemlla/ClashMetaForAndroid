...
假设审查一个支付模块发现：未验证的折扣码可重复使用（影响 4、可能性 4、爆炸半径 2——影响单笔交易、变化热度 1——已存在半年未修复、恢复惩罚 3——已扣款难追回）：

`Priority Index = 4 × 4 + 2 + 1 + 3 = 22`

22 分落在 18 以上紧急候选波段，应直接进入 Critical 候选。记录为：

| ID | 标题 | I | L | B | H | R | Index | 波段 | 置信度 |
|----|------|---|---|---|---|---|---|------|--------|
| F-001 | 折扣码重复使用无幂等校验 | 4 | 4 | 2 | 1 | 3 | 22 | 紧急 | 中 |

再例：一个只读 API 端点缺少节流（影响 1、可能性 3、爆炸半径 1——单个用户滥用、变化热度 0、恢复惩罚 1——重启后恢复）：

`Priority Index = 1 × 3 + 1 + 0 + 1 = 5`

5 分落在 0–7 低波段，标记为 Low 或 Medium。
...
8 个审计检查维度与 7 个评分维度并非一一对应。以下映射表说明每个审计维度主要贡献到哪些评分维度，以及跨维度发现如何处理：

| 审计维度 | 主要评分维度 | 次要贡献 | 说明 |
|----------|-------------|----------|------|
| 架构与领域模型 | Design | Maintainability, Stability | 边界和依赖影响可维护性，热点和容错影响稳定性 |
| 代码与局部设计 | Maintainability, Design | Testing | 可读性/可维护性直接评分；测试覆盖的代码质量影响 Testing |
| 数据、API 与契约 | Release, Stability | Design | 兼容性影响 Release 发布能力；契约破坏影响稳定性 |
| 产品界面与可访问性 | Design | 无 | 功能适合性、UX 和可访问性归入 Design |
| 安全与可靠性 | Security, Stability | 无 | 安全单独评分；可靠性归入 Stability |
| 性能与可观测性 | Performance | Stability | 性能和可观测性直接评分；容量不足影响稳定性 |
| 测试与交付 | Testing, Release | 无 | 测试质量直接评分；构建/发布流水线归入 Release |
| 演进（文档/所有权/团队） | Maintainability | Release | 文档和知识连续性影响可维护性 |

**跨维度发现处理规则：**
- 一个发现影响多个评分维度时，在主要维度的评分依据中体现，并在相关维度交叉引用。
- 例如：一个缺少文档的安全相关配置项，在 Security 中记录行为影响，在 Maintainability 中记录文档缺失。
- 确保同一发现不在两个维度中重复计算为独立问题。
